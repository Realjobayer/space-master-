...
# Reset game function
def reset_game():
    global player_pos, player_angle, player_health, player_shield, bullets, enemies, enemy_bullets
    global kill_count, shield_drops, boss, boss_health, boss_pos, boss_alive
...
while running:
    ...
    player_bullet_hit = check_player_bullet_collision(player_pos, enemy_bullets)
    if player_bullet_hit:
        enemy_bullets.remove(player_bullet_hit)
        if player_shield > 0:
            player_shield -= 10
            if player_shield < 0:
                player_health += player_shield
    ...
    if shield_pickup:
        ...
        player_shield = min(player_shield + 25, 100)  # Increase shield up to a maximum of 100
    ...
    if boss_alive:
        ...
        # Check for boss collision with player's bullets
